...
<xsl:stylesheet
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p>We do not use the fn:serialize for a number of reasons, mainly since
    ...
    <xsl:function name="jt:returnInitialSpace" as="xs:integer" new-each-time="no">
        ...
        <xsl:variable name="spaces" select="for $l in $lastLines return jt:countLeadingSpace($l)" as="xs:integer*"/>
    ...
    <xsl:function name="jt:countLeadingSpace" as="xs:integer">
    ...
    <xd:doc>
        <xd:desc>A simple utility function takes in a tag and returns a boolean value whether or not the element should be
    ...
    <xsl:function name="jt:isSelfClosed" as="xs:boolean" new-each-time="no">
        <xsl:param name="tag"/>
